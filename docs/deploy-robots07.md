...
### Категории: база в туннель

Категории лежат в их PostgreSQL на Windows, а в их API их только читают —
создать, переименовать или перенести нельзя. Заказчику это нужно, поэтому
сервис сайта ходит в базу напрямую. Ровно за этим, больше ни за чем:
товары, цены и остатки по-прежнему берутся через их API.

Сегодня это безопасно: синхронизация 1С меняет категории только если в
классификаторе больше одной группы, а там одна («Товар») — категории она не
трогает (см. README бэкенда). Если в 1С однажды заведут настоящие группы,
дерево начнёт приходить оттуда, и заведённое руками стоит будет сверить.

**На Windows** — пустить VPS к базе по туннелю. В `postgresql.conf`:

```
listen_addresses = 'localhost,10.8.0.2'
```

В `pg_hba.conf` — строкой ниже остальных `host`:

```
host    shop    shop    10.8.0.1/32    scram-sha-256
```

Затем перезапустить службу PostgreSQL и открыть порт только туннелю:

```powershell
Restart-Service postgresql-x64-18
New-NetFirewallRule -DisplayName "PostgreSQL из туннеля" -Direction Inbound `
  -Protocol TCP -LocalPort 5432 -RemoteAddress 10.8.0.1 -Action Allow
```

Наружу база при этом не смотрит: 10.8.0.1 — адрес VPS внутри WireGuard, и
попасть на него можно только через туннель.

**На VPS** — вписать адрес базы в сервис и поставить драйвер:

```bash
.venv/bin/pip install -r services/site/requirements.txt
sed -i "s/ЗАМЕНИТЕ_ПАРОЛЬ_БАЗЫ/<пароль пользователя shop>/" \
       /etc/systemd/system/shop-site.service
systemctl daemon-reload && systemctl restart shop-site
```

Проверка — с VPS:

```bash
curl -s -u admin:пароль -X POST https://robots07.com/site-api/admin/categories \
     -H 'Content-Type: application/json' -d '{"name":"Проверка связи"}'
```

Ответ `{"id":..., "name":"Проверка связи", ...}` означает, что всё сошлось;
категорию после этого удалить из админки. Если вместо этого приходит
«Сервису не задан адрес базы каталога» — не подхватилась строка в unit;
«База каталога не отвечает» — не пускает `pg_hba.conf` или файрвол.
