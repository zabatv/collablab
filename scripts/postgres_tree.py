...
def build_sql(tree, products):
    ...
    add('')
    add('-- Файл в UTF-8, а psql в консоли Windows по умолчанию считает, что')
    add('-- читает WIN1251, и спотыкается на первой же кириллице. Две строки')
    add('-- ниже снимают зависимость от настроек консоли.')
    add('\\encoding UTF8')
    add("SET client_encoding TO 'UTF8';")
