...
слайдера на главной, текста под заголовком раздела, файлов раздела,
статистики просмотров и разбора готовности каталога у него нет и не будет —
из 1С такие вещи не приходят. Всё это держит наш маленький сервис на этом же
VPS, а правит заказчик в админке, как раньше.
...
(`logos/`), слайды (`banners/`), документация к товарам (`docs/`) и файлы
разделов (`category/`). Это единственное, что на VPS стоит копировать:
каталог живёт на Windows.

Видео раздела весит столько же, сколько видео товара, поэтому в nginx для
`/site-api/` стоит `client_max_body_size 300m`. Если конфиг ставился раньше,
чем появились файлы разделов, там осталось `32m` — тогда загрузка ролика
оборвётся ответом 413:

```bash
sed -i '/location \/site-api\//,/}/ s/client_max_body_size 32m;.*/client_max_body_size 300m;/' \
       /etc/nginx/sites-available/robots07.com
nginx -t && systemctl reload nginx
```

### Категории: недостающий кусок бэкенда

В их API категории только читают — создать, переименовать или перенести
нельзя. Заказчику это нужно, поэтому недостающие эндпоинты написаны нами и
лежат в `frontend/backend/`: один новый файл в их проекте и одна строка
подключения. Установка — `frontend/backend/README.md`, делается на Windows.

Пока это не сделано, кнопки категорий в админке отвечают
`Cannot POST /categories`. Всё остальное работает как и раньше.

Сегодня заведённое руками дерево безопасно: синхронизация 1С меняет
категории только если в классификаторе больше одной группы, а там одна
(«Товар») — категории она не трогает (см. README бэкенда). Если в 1С однажды
заведут настоящие группы, дерево начнёт приходить оттуда, и заведённое
руками стоит сверить.
