...
Остаётся **перезапустить сервер админки** — он работает службой
`ShopCatalogAdmin` (порт 3001). В PowerShell от администратора:

```powershell
Restart-Service ShopCatalogAdmin
```

`ShopCatalogAPI` (порт 3000, товары) трогать не нужно. После перезапуска
категории заводятся и правятся из админки сайта.
